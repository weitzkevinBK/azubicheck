...
function App() {
  ...
  async function creditFullTheoryDay(blockId, studentId, date, options = {}) {
    ...
    const setting = getTheoryDaySetting(date, override)
    const existing = store.theoryAttendances.find(
      (item) => item.blockId === blockId && item.studentId === studentId && item.date === date,
    )
    const patch = {
      blockId,
      courseId: block.courseId,
      studentId,
      date,
      checkInTime: existing?.checkInTime || setting.officialStartTime,
      checkOutTime: existing?.checkOutTime || setting.officialEndTime,
      checkoutChoice: 'teacherManualFullDay',
      calculatedHours: setting.fullCreditHours,
      adjustedHours: setting.fullCreditHours,
      status: 'manual',
      enteredBy: currentUser.id,
    ...
    if (firebaseEnabled) {
      if (existing) await updateDoc(doc(db, collectionMap.theoryAttendances, existing.id), patch)
      else {
        const id = createId('theory')
  ...
  const courseBulkAbsences = useMemo(() => {
    ...
    courseStudents.forEach((student) => {
      getAbsenceItems(store, student)
        .filter((entry) => (entry.status === 'Teilfehlzeit' || entry.status === 'Fehltag') && entry.blockId)
        .forEach((entry) => {
          const key = `${entry.blockId}|${entry.startDate}|${entry.status}`
    ...
    return Array.from(grouped.values())
      .filter((group) => {
        if (group.status === 'Teilfehlzeit') return group.studentIds.length >= threshold
        return group.studentIds.length === courseStudents.length
      })
      .sort((a, b) => b.date.localeCompare(a.date))
      ...
      <section className="panel">
        {selectedStudent && summary ? (
          <>
            ...
            {!!courseBulkAbsences.length && (
              <div className="bulk-absence-list">
                {courseBulkAbsences.map((group) => (
                  <div className="bulk-absence-box" key={`${group.blockId}-${group.date}-${group.status}`}>
                    <AlertTriangle size={18} />
                    <span>
                      {group.status === 'Fehltag'
                        ? `Alle Azubis haben am ${formatDate(group.date)} einen Fehltag.`
                        : `${group.studentIds.length} Azubis haben am ${formatDate(group.date)} eine Teilfehlzeit.`}
                    </span>
                    <button
                      type="button"
                ...
                {details.map((entry) => (
                  <div className="row attendance-detail-row" key={`${entry.startDate}-${entry.endDate}-${entry.source}-${entry.status}`}>
                    ...
                    <span className="row-action">
                      {entry.status === 'Teilfehlzeit' && entry.blockId ? (
                        <button
                          type="button"
                          className="icon-button compact-action"
                          title="Vollen Tag gutschreiben"
                          onClick={() => creditFullTheoryDay(entry.blockId, selectedStudent.id, entry.startDate)}
                        >
                          <Check size={16} />
                        </button>
                      ) : '-'}
